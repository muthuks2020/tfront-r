...
// import {DefaultField} from '../Misc/reduxBootstrap'
import {Material} from '../Spinner'
import {validate, asyncValidate} from '../Register/registerValidation'
// import {TextInput} from '../Misc/MaterialInput'

// const registerField = ({ input, label, type, meta: { touched, error, warning } }) => (
//   <div className={touched && error ? 'has-error' : null}>
//     <label>{label}</label>
//     <input {...input} type={type} className="form-control input-lg"/>
//     {touched && ((error && <p className="help-block">{error}</p>) || (warning && <span>{warning}</span>))}
//   </div>
// )

@reduxForm({
  form: 'registerForm',
  fields: ['password', 'email', 'display_name', 'confirm_password'],
  touchOnChange: true,
  asyncBlurFields: ['email', 'display_name'],
  validate,
  asyncValidate
})
@connect(
...
export default class RegisterView extends React.Component {
  ...
  handleSubmit(data) {
    this.props.registerUser(data.email, data.password, data.display_name)
  }
          ...
          >
            ...
            <div className="row">
              <div className="col-xs-12 col-md-6">
                {this.props.auth.isRegistering
                  ? <Material />
                  : <FlatButton type="submit" fullWidth={true}>
                      Register
                    </FlatButton>}
              </div>
            </div>
